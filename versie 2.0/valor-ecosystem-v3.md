...
**EcosystemAgent.** Een coalitie van autonome, onafhankelijke organisaties die samenwerken aan een gedeelde publieke dienst zonder hiërarchisch gezag over elkaar. In VALOR-O geformaliseerd als `nexus:EcosystemAgent` — een `valor:CollectiveIntentionalCommunity` in de conatieve modus, geconstitueerd door een `nexus:CollaborationCommitment`. De capabilities van een EcosystemAgent zijn per definitie emergent. De EcosystemAgent baseert zijn mandaat op de `valor:IssueCommunity` die het betrokken Issue erkent: hij adresseert via `nexus:addressesIssue` het Issue dat door de bredere gemeenschap als problematisch wordt ervaren. Zie ook: IssueCommunity, CollectiveIntentionalCommunity.

**IssueCommunity.** De gemeenschap van agents die een Issue als problematisch erkent — geconstitueerd door gedeeld `ufoc:Concern`. In VALOR-O geformaliseerd als `valor:IssueCommunity` — een `valor:CollectiveIntentionalCommunity` in de cognitieve modus. De IssueCommunity bestaat zodra agents elkaars zorg wederzijds erkennen; ze hoeft niet te handelen. Ze is typisch breder dan de EcosystemAgent die handelt: burgers, uitvoerders, maatschappelijke organisaties en beleidsmakers die samen een situatie als problematisch constitueren. De IssueCommunity is de normatieve legitimeringsbron van de EcosystemAgent.

**CollectiveIntentionalCommunity.** De abstracte ontologische category die het gemeenschappelijke patroon van IssueCommunity en EcosystemAgent vastlegt: een groepering van agents geconstitueerd door een gedeelde intentionele toestand met betrekking tot een externe situatie in de wereld. Twee modi: de cognitieve modus (IssueCommunity — erkent) en de conatieve modus (EcosystemAgent — handelt). Grondslag: Searle (1995), collectieve intentionaliteit als constitutief mechanisme voor sociale realiteit.
...
- **`nexus:EcosystemAgent`** (`valor:CollectiveIntentionalCommunity` / conatieve modus, stereotype `subkind`) — Een coalitie van autonome organisaties die samenwerken aan een gedeelde publieke dienst (DD-091). Geconstitueerd door een `nexus:CollaborationCommitment`; bestaat door wederzijdse erkenning van de leden. Geen externe institutionele erkenning vereist; geen intern hiërarchisch gezag. De leden zijn `ufoc:InstitutionalAgent`s. Ontologisch onderscheid van een enkelvoudige organisatie: een EcosystemAgent kan geen verordening uitvaardigen of een arbeidscontract sluiten — zijn handelingsmacht is uitsluitend gebaseerd op het multilaterale commitment van de leden. Adresseert via `nexus:addressesIssue` het Issue dat zijn democratische legitimering vormt: de EcosystemAgent handelt NAMENS de `valor:IssueCommunity` die dat Issue erkent.
...
Een `valor:Issue` is een `ufoc:SocialObject` — een maatschappelijk vraagstuk dat bestaat doordat een `valor:IssueCommunity` een gedeeld `ufoc:Concern` heeft over een onderliggende `gufo:Situation`. Het Issue is daarmee een sociaal construct (het bestaat in virtue van collectieve intentionaliteit), nadrukkelijk onderscheiden van de situatie in de werkelijkheid die aanleiding geeft. Die situatie wordt vastgelegd via `valor:concernedWithSituation`.

De `valor:IssueCommunity` die het Issue constitueert is een `valor:CollectiveIntentionalCommunity` in de cognitieve modus (DD-091): geconstitueerd door gedeeld Concern, niet door een formele akte of institutionele erkenning. Haar relatie tot het Issue wordt vastgelegd via `valor:hasConcernAbout`. De IssueCommunity is tevens de normatieve legitimeringsbron van de `nexus:EcosystemAgent` die het Issue adresseert: het ecosysteem handelt in opdracht van de gedeelde zorg van de bredere gemeenschap, vastgelegd via `nexus:addressesIssue`.

Een Issue heeft een domein, een scope-beschrijving, en is via `valor:isAddressedInDesignSpace` gelinkt aan de DesignSpace(s) waarin het wordt geanalyseerd en ontworpen.
...
### 3.5 Haalbaarheid als bijzondere Tessera-categorie

CapabilityGap-Tesserae en FeasibilityAssessment-Tesserae nemen een bijzondere positie in binnen de claim-architectuur.

Een **CapabilityGap** is altijd een as-is Tessera: ze beschrijft een tekortkoming in de huidige situatie ten aanzien van een in het alternatief vereiste capability. Ze is dus inherent relatief aan een specifiek to-be alternatief: de gap bestaat pas wanneer een CapabilityRequirement formeel is vastgesteld voor dat alternatief. Dit maakt CapabilityGap-Tesserae niet enkel een technische observatie maar een ontwerpgegeven: het ontwerp zelf creëert de norm waaraan de as-is werkelijkheid wordt afgemeten.

Een **FeasibilityAssessment** is een bijzondere Tessera omdat ze een geaggregeerd oordeel uitdrukt over een volledige set CapabilityGap-Tesserae voor één alternatief. Ze kan niet worden voorgesteld door één deelnemer alleen — ze is het resultaat van een bewuste synthese door de groep, gefaciliteerd door de Delibera-agent. Haar `epistemicStatus = Accepted` is een noodzakelijke voorwaarde voor een faseovergang, niet slechts een aanbeveling. Dit maakt de FeasibilityAssessment functioneel equivalent aan een faseovergang-gate: haalbaarheid is een formele democratische beslissing, geen technische achterkamerbeoordeling.
...
**Theoretische basis.** Causal Loop Diagrams (Forrester/Meadows), Causal Inference (Pearl), systeemdenken, PAMS-onzekerheidstaxonomie (Enserink et al. 2022).

**Wat wordt gemodelleerd.** Oorzaak-gevolgketens, versterkende en dempende terugkoppelingen, vertraagde effecten, systeemgrenzen. Causa maakt zichtbaar welke mechanismen het Issue in stand houden en welke interventies welke effecten hebben. Een fundamentele dimensie van Causa is **onzekerheid**: causale claims variëren van statistisch onderbouwde relaties tot expertintuïties in gebieden van diepe onzekerheid. Elke causale claim draagt daarom een `valor:uncertaintyLevel` conform de PAMS-taxonomie.

**Kernconcepten in VALOR-O.** `causa:CausalVariable` als `gufo:Quality`-type dat inhereert aan een object of situatie, met een `causa:polariteitsAnnotatie` en optionele vertragingsparameter. `causa:CausalClaim` als specialisatie van `ufoc:Belief` met een `causa:CausalSituation` als propositionele inhoud — geen nieuwe foundationele types nodig. `causa:FeedbackLoop` als `gufo:Situation` die een cyclisch relatiepatroon constitueert; detecteerbaar via SPARQL property paths. `causa:ExplanatoryModel` als een samenhangende set CausalClaims die een coherente verklaring voor het Issue bieden; meerdere ExplanatoryModels kunnen parallel bestaan en worden betwist (probleemfase-deliberatie). `causa:SolutionModel` als een set CausalClaims over het effect van een interventie; subject van oplossingsfase-deliberatie. Interventies als `gufo:EventType`s die de waarde van een CausalVariable beïnvloeden.

**CAPAX-overlay in Causa.** Een interventie in Causa correspondeert altijd met een TransactionType in Acta. Wanneer de Causa-agent constateert dat een interventie een bepaald uitvoerend vermogen vereist, kan zij een CapabilityRequirement-Tessera voorstellen. De CAPAX-overlay toont bij elke interventie of er een gedekte of ongedekte CapabilityRequirement is.

**Visuele notatie.** Cirkels of rechthoeken voor variabelen, pijlen met `+`/`-` voor causale relaties, dubbele pijlen voor vertraagde effecten, `R`/`B` labels voor reinforcing en balancing loops. Onzekerheidsniveau als kleurcodering op de relatiepijlen (groen = statistisch onderbouwd, rood = diepe onzekerheid).
...
**Tessera-voorbeelden in Causa.** "Schuldenproblematiek leidt tot verminderde arbeidsparticipatie" (as-is, causaliteitsrelatie, onzekerheidsniveau: Scenario). "De interventie 'geïntegreerde schuldhulpverlening' vermindert de schuldenlast structureel" (to-be, interventie-effect, onzekerheidsniveau: DeepUncertainty).
...
3. **Als gate-Tessera in Delibera** — de FeasibilityAssessment is een verplichte Tessera bij elke faseovergang. Haalbaarheid is daarmee een formele democratische beslissing, zichtbaar en betwistbaar voor alle deelnemers in het perspectief dat voor iedereen toegankelijk is.

De **Acta-agent** breidt haar vraagstrategie uit met capability-vragen: "Welke expertise is vereist voor deze transactie? Heeft de betrokken organisatie die expertise in huis?" De **Socia-agent** stelt capability-vragen op actor-niveau. De **Delibera-agent** faciliteert het haalbaarheidsordeel bij faseovergangen.

Deze architectuur borgt dat haalbaarheid een **doorlopende lens** is op het hele ontwerp — niet een afrondende checklist die achteraf wordt ingevuld.
...
Faseovergang-criterium: elk alternatief is voldoende uitgewerkt voor vergelijking en beoordeling. SHACL-validatie in Forma slaagt voor alle aangenomen Tesserae. De FeasibilityAssessment per alternatief heeft status `Accepted`.
...
De Delibera-agent controleert automatisch of voor alle actieve alternatieven een FeasibilityAssessment met status `Proposed` of `Accepted` aanwezig is voordat de stemronde wordt geopend. Bij ontbrekende FeasibilityAssessments signaleert de agent dit als een procedurele onvolledigheid.
...
De CAPAX-engine is een backend-component die de haalbaarheidslaag beheert:

- **Gap-detectie**: SPARQL-query over alle TransactionTypes × ActorRole-toewijzingen × CapabilityRequirements × as-is Capabilities — levert een lijst van ongedekte requirements als potentiële CapabilityGaps.
- **FeasibilityAssessment-aggregatie**: berekent het geaggregeerde haalbaarheidsordeel per alternatief op basis van de set CapabilityGap-Tesserae met status `Accepted`.
- **Gate-check**: valideert bij een faseovergang of alle actieve alternatieven een geaccepteerde FeasibilityAssessment hebben; blokkeert de faseovergang als dit niet het geval is.
- **CAPAX-overlay-data**: levert de capability-statusdata voor de Acta- en Socia-overlays via de Perspective Query API.
...
*(9) `00l` NEXUS — Ecosysteem capabilities (v0.3)* — `CollaborationCondition` als `SystemSituation` met drie constitutieve lagen.

### 12.4 Fase -1c: Formalisering niet-eerder-geformaliseerde theorieën (6 maanden) — GROTENDEELS AFGEROND

**Causal Loop Diagrams** (`00h` CAUSA) — geformaliseerd met `CausalClaim` als `ufoc:Belief`-specialisatie, `CausalSituation` als propositionele inhoud, PAMS-onzekerheidsannotaties, twee-fase/twee-niveau deliberatiestructuur (ExplanatoryModel vs. SolutionModel).
...
| **-1b** | Integratie bestaande formaliseringen | Grotendeels afgerond | Modules 00b–00l (UFO-C v2, ACTA, CAUSA, SOCIA, CAPAX v0.2, AXIA-VSD, NEXUS v0.4) |
| **-1c** | Formalisering niet-geformaliseerde theorieën | Grotendeels afgerond | Modules 00h (CAUSA/CLD), 00i (SOCIA/i*), 00m (SYSONT), 00n (VALOR-CORE/DD-091) |
...
| **Fase 4** | Causa | 8 wk | Causale view (CLD-notatie), onzekerheidsniveau-annotaties, feedback loop detectie via SPARQL, Causa-agent |
...
flowchart TB
  ...
  classDef valorcore   fill:#1a0a2e,stroke:#c084fc,color:#f3e8ff,font-size:11px
  ...
  %% ─── VALOR-CORE (00n — DD-091) ──────────────────────────────────────────
  subgraph VALORCORE["00n VALOR-CORE — CollectiveIntentionalCommunity (DD-091)"]
    CollIntCom["CollectiveIntentionalCommunity\n«category»\ngedeelde intentionaliteit\ngericht op externe Situation\n(Searle 1995)"]:::valorcore
    IssueCommunity["IssueCommunity\n«subkind»\nCOGNITIEVE MODUS\nconstitutief moment:\ngedeeld Concern\nexisteert: erkent"]:::valorcore
    IssueNode["Issue\n«kind» ← SocialObject\ngeconstitueerd door\nIssueCommunity via\ngedeeld Concern"]:::valorcore
  end

  %% ─── NEXUS ───────────────────────────────────────────────────────────────
  subgraph NEXUS["NEXUS — Ecosysteem Capabilities (v0.4)"]
  ...
  %% DD-091 — CollectiveIntentionalCommunity-taxonomie
  CollIntCom -.->|"spec."| Agent
  IssueCommunity -.->|"cognitieve\nmodus"| CollIntCom
  EcosystemAgent -.->|"conatieve\nmodus"| CollIntCom
  IssueCommunity -->|"hasConcernAbout\n(constitueert)"| IssueNode
  IssueNode -->|"concernedWith\nSituation"| Situation
  EcosystemAgent -- "addressesIssue\n(legitimering)" --> IssueNode
...
- Searle, J.R. (1995), *The Construction of Social Reality*, Free Press — collectieve intentionaliteit als constitutief mechanisme (DD-091)
