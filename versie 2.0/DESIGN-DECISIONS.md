...
## Moduleoverzicht (bijgewerkt v0.9)
...
| 00g | `00g-acta.ttl` | ACTA DEMO Transactions | ✓ Compleet |
| 00h | `00h-causa.ttl` | CAUSA Causal Loop Diagrams | ✓ Compleet |
| 00i | `00i-socia.ttl` | SOCIA Actor Analysis (i*) | ✓ Compleet |
| 00j | `00j-capax.ttl` | CAPAX Capability & Feasibility v0.2 (DD-084) | ✓ Compleet |
...
- Globale SPARQL-queries over de volledige VALOR-O graph schrijven (emergentiepatroon-detectie, gap-keten, Tessera-audit trail, **legitimiteitsaudit DD-091**)
- `00m-sysont` instantiëren met het Spotify-patroon uit Calhau et al. (2023) §6 als validatie-voorbeeld
- Review CAPAX v0.2 SHACL: `capax:CapabilityDevelopmentNeed.developmentApproach` gecontroleerd vocabulaire uitbreiden met `'StructuralRedesign'` en `'GovernanceRedesign'`
- SHACL voor `00n-valor-core`: minimaal (a) `EcosystemAgent` MUST have ≥1 `addressesIssue`, (b) `IssueCommunity` MUST have ≥1 `hasConcernAbout`, (c) `Issue` MUST have `concernedWithSituation`
